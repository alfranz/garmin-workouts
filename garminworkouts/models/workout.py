    ...
    def _generate_description(self):
        ...
        for step in flatten_steps:
            ...
            if power_watts and duration_secs:
                ...
                xs = functional.concatenate(
                    xs, functional.fill(power_watts, duration_secs)
                )
    ...
    def _steps_recursive(self, steps_config, step_order, child_step_id):
        ...
        for repeats, step_config in steps_config_agg:
            ...
            if isinstance(step_config, list):
                ...
                steps.append(
                    self._repeat_step(
                        repeat_step_order, repeat_child_step_id, repeats, nested_steps
                    )
                )
            else:
                steps.append(
                    self._interval_step(step_config, child_step_id, step_order)
                )
